...
const API = {
  ...
  calendar: {
    ...
    getPosts: (params = {}) => {
      ...
      const endpoint = '/forum.php' + (qs ? `?${qs}` : '');
      return apiCall(endpoint);
    },
    getPost: (id) => apiCall(`/forum.php/${id}`),
    createPost: (data) => apiCall('/forum.php', {
      method: 'POST',
      body: JSON.stringify(data)
    }),
    updatePost: (id, data) => apiCall(`/forum.php/${id}`, {
      method: 'PUT',
      body: JSON.stringify(data)
    }),
    deletePost: (id) => apiCall(`/forum.php/${id}`, { method: 'DELETE' }),
    getReplies: (postId) => apiCall(`/forum.php/${postId}/replies`),
    createReply: (postId, data) => apiCall(`/forum.php/${postId}/replies`, {
      method: 'POST',
      body: JSON.stringify(data)
    })
  },
